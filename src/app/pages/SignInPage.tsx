...
export default function SignInPage() {
  ...
  const { login, completeExternalLogin, loading, error: authError } = useAuth();

  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const token = params.get("token") || params.get("oauth_token") || params.get("access_token");
    if (!token) {
      return;
    }
  ...
  const handleGoogleSignIn = async () => {
    setLocalError("");
    window.location.href = `${BACKEND_URL}/api/auth/oauth/google/start`;
  };

  const handleGitHubSignIn = async () => {
    setLocalError("");
    window.location.href = `${BACKEND_URL}/api/auth/oauth/github/start`;
  };
      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-[#E2E8F0] p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  className="w-4 h-4 rounded border-2 border-[#E2E8F0] text-[#2563EB] focus:ring-2 focus:ring-[#2563EB] focus:ring-offset-0"
                />
                <span
                  style={{
                    fontFamily: "'Inter', sans-serif",
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              className="flex items-center justify-center gap-2 border-2 border-[#E2E8F0] hover:border-[#2563EB] hover:bg-[#EFF6FF] py-3 rounded-xl transition-all"
              style={{
                fontFamily: "'Inter', sans-serif",
                fontWeight: 600,
                fontSize: "0.875rem",
